...
CREATE MATERIALIZED VIEW kmx_vehicles AS
 SELECT (vehicle_jsonb.v -> 'StockNumber'::text) AS stocknumber,
   ...
   FROM ( SELECT jsonb_array_elements((kmx_doc.doc -> 'Results'::text)) AS v
           FROM kmx_doc) vehicle_jsonb
  GROUP BY (vehicle_jsonb.v -> 'Vin'::text), (vehicle_jsonb.v -> 'Year'::text), (vehicle_jsonb.v -> 'Make'::text), (vehicle_jsonb.v -> 'Model'::text), (vehicle_jsonb.v -> 'Description'::text), (vehicle_jsonb.v -> 'Price'::text), (vehicle_jsonb.v -> 'ExteriorColor'::text), (vehicle_jsonb.v -> 'Miles'::text), (vehicle_jsonb.v -> 'DriveTrain'::text), (vehicle_jsonb.v -> 'Transmission'::text), (vehicle_jsonb.v -> 'IsSold'::text), (vehicle_jsonb.v -> 'StockNumber'::text)
  WITH NO DATA;
